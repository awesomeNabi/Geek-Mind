                                             ...
                                             tf_listener_(std::make_shared<tf2_ros::TransformListener>(tf_buffer_))
{
    ...
    const auto map_publish_start = std::chrono::steady_clock::now();
    map_publish_timer_ = this->create_wall_timer(std::chrono::seconds(2), [this, map_publish_start]() {
        if (std::chrono::steady_clock::now() - map_publish_start > std::chrono::seconds(30))
        {
            map_publish_timer_->cancel();
            RCLCPP_INFO(this->get_logger(), "stop republishing /map after startup window");
            return;
        }
